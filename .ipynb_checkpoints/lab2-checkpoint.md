...
10. Once you run this command you should add this new file to repo and make a commit, so while you're still in the same directory, run the following commands will do this:
    `git add breadcrumps.txt`
    `git commit -m "a meaningful message to remember later"`
    `git push`
    
**Tip: afterwards, when I say "push to Git", I mean run the command below!by running these commands, you basically will have a snapchat of what you have been so far in this game!**
...
18. Let's go and do that now with `cd ..`, pick up the treasure from the cellar, and then come back to the armoury.
...
18. Have fun! More dungeon rooms will open up as you complete certain tasks, kill certain monsters, and read specific scrolls and tomes. Have fun with it, this is your opportunity to practice your Terminal skills - it sure beats the way I learned Terminal commands (blindly typing in commands in a black box until I got it to do what I wanted!). There is no need to hit **every** room in the dungeon, but do make a concerted effort ( you should at least have 3 'breadcrumps.txt' files  and 3 commits repsectively in totall) and you'll likely get most of the marks for this lab. 
